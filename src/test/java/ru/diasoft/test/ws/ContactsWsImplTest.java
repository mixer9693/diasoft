...
class ContactsWsImplTest {
    ...
    void getContactByIdWithValidParam(){
        Contact contact = port.getById(MockData.CONTACT_1_ID);
    ...
    void updateContactWithValidParams(){
        ...
        try{
            updatedContact = port.getById(MockData.CONTACT_1_ID);
    ...
    void removeContactWithValidParams(){
        Contact contact = port.getById(MockData.CONTACT_2_ID);
        port.remove(contact);

        Contact removedContact = null;
        try {
            removedContact  = port.getById(MockData.CONTACT_2_ID);
        }catch (Exception e){
            fail("Method execution aborted due to error getting contact: " + e.getMessage());
        }
